...
{
	/// <summary>
	/// Реализация алгоритма генерации псевдослучайной последовательности (Pseudorandom Function, PRF) по ГОСТ Р 34.11-2012-512.
	/// </summary>
	public sealed class Gost3411_2012_512Prf : IDisposable
	{
		/// <summary>
		/// URI алгоритма для использования в протоколе WS-Trust в качестве алгоритма вычисления ключа.
		/// </summary>
		public const string Gost3411PrfComputedKeyUrl = "urn:ietf:params:xml:ns:cpxmlsec:algorithms:ck-p-gostr3411-2012-512";

		/// <summary>
		/// URI алгоритма для использования в протоколах на базе WS-SecureCoveration.
		/// </summary>
		public const string Gost3411PrfKeyDerivationUrl = "urn:ietf:params:xml:ns:cpxmlsec:algorithms:dk-p-gostr3411-2012-512";
		...
		private Gost3411_2012_512Prf(byte[] label, byte[] seed)
		{
			if (label == null)
			{
		...
		/// Конструктор.
		/// </summary>
		/// <param name="key">Симметричный ключ ГОСТ 28147 для вычисления HMAC на основе алгоритма ГОСТ Р 34.11-2012-512.</param>
		/// <param name="label">Метка для порождения ключей (аргумент label функции PRF).</param>
		/// <param name="seed">Начальное число для порождения ключей (аргумент seed функции PRF).</param>
		/// <exception cref="ArgumentNullException"></exception>
		[SecuritySafeCritical]
		public Gost3411_2012_512Prf(Gost28147 key, byte[] label, byte[] seed) : this(label, seed)
		{
			if (key == null)
			{
			...
			_buffer = new byte[_labelAndSeed.Length + 64];
		...
		/// <exception cref="ArgumentNullException"></exception>
		[SecuritySafeCritical]
		public Gost3411_2012_512Prf(byte[] key, byte[] label, byte[] seed) : this(label, seed)
		{
			if (key == null)
			{
				throw ExceptionUtility.ArgumentNull("key");
