...
namespace GostCryptography.Cryptography
{
	...
	/// </summary>
	sealed class GostSharedSecretAlgorithm : GostSharedSecret
	{
		[SecurityCritical]
        ...
        private readonly GostAlgorithmType _algType;
		...
		public override byte[] EncodeKeyExchange(SymmetricAlgorithm keyExchangeAlgorithm, GostKeyExchangeExportMethod keyExchangeExportMethod)
		{
			...
			if (keyExchangeAlgorithm is Gost28147)
			{
				using (var gostKeyExchangeAlgorithm = new Gost28147SymmetricAlgorithm())
				{
					return gostKeyExchangeAlgorithm.EncodePrivateKey((Gost28147)keyExchangeAlgorithm, keyExchangeExportMethod);
				}
			}
		...
		private byte[] EncodeKeyExchangeInternal(Gost28147SymmetricAlgorithm keyExchangeAlgorithm, int keyExchangeExportAlgId)
		{
			...
			try
			{
				keyExchangeHandle = CryptoApiHelper.ImportAndMakeKeyExchange(_provHandle, _keyExchangeParameters, _keyHandle, _algType);
