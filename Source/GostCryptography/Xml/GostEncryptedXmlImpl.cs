...
//TODO Security using System.Security.Policy;
using System.Xml;
...
namespace GostCryptography.Xml
{
	sealed class GostEncryptedXmlImpl : EncryptedXml
	{
		...
		public new EncryptedData Encrypt(XmlElement element, X509Certificate certificate)
		{
			if (element == null)
			{
            ...
            if (!string.Equals(oid, Constants.OID_GR3410_2001, StringComparison.OrdinalIgnoreCase) &&
                !string.Equals(oid, Constants.OID_GR3410_12_256, StringComparison.OrdinalIgnoreCase) &&
                !string.Equals(oid, Constants.OID_GR3410_12_512, StringComparison.OrdinalIgnoreCase))
			{
				return base.Encrypt(element, certificate);
			}
            ...
            {
                EncryptionMethod = new EncryptionMethod(oid == Constants.OID_GR3410_12_256 ? GostEncryptedXml.XmlEncGost2012_256KeyTransportUrl :
                                                        oid == Constants.OID_GR3410_12_512 ? GostEncryptedXml.XmlEncGost2012_512KeyTransportUrl :
                                                        GostEncryptedXml.XmlEncGostKeyTransportUrl)
            };
			...
			encryptedKey.CipherData.CipherValue = EncryptKey(encriptionKey, publicKey as Gost3410);
		...
		public override SymmetricAlgorithm GetDecryptionKey(EncryptedData encryptedData, string symmetricAlgorithmUri)
		{
			...
			if (encryptedData.KeyInfo != null)
			{
				...
				foreach (var keyInfo in encryptedData.KeyInfo)
				{
					...
					{
						var idValue = Utils.ExtractIdFromLocalUri(((KeyInfoRetrievalMethod)keyInfo).Uri);
				...
				foreach (var keyInfo in encryptedKey.KeyInfo)
				{
					...
					if (keyInfo is KeyInfoName)
					{
						...
						if (keyAlgorithm != null)
						{
							...
							else if (keyAlgorithm is Gost3410)
							{
								decryptionKey = DecryptKeyClass(encryptedKey.CipherData.CipherValue, (Gost3410)keyAlgorithm);
							}
						}
					...
					if (keyInfo is KeyInfoX509Data)
					{
						...
						var certificates = Utils.BuildBagOfCerts((KeyInfoX509Data)keyInfo, CertUsageType.Decryption);
						...
						foreach (var certificate in certificates)
						{
							...
							else if (privateKey is Gost3410)
							{
								decryptionKey = DecryptKeyClass(encryptedKey.CipherData.CipherValue, (Gost3410)privateKey);
							}
						}
		...
		private static SymmetricAlgorithm DecryptKeyClass(byte[] keyData, SymmetricAlgorithm algorithm, string symmetricAlgorithmUri, string encryptionKeyAlgorithm)
		{
			...
			var gost28147 = algorithm as Gost28147;
